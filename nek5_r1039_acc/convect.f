...
!!$ACC  DATA COPY(phx,vx,vxlag,)                                                               
         call char_conv_acc(phx,vx,vxlag,hmsk,c_vx,ct_vx,gsh_fld(1))
!!$ACC END DATA                                                                                
...
!     dt                                                                                       
      ...
      tau = time-vlsum(dtlag,nbd)         ! initialize time for u^n-k                          
      call int_vel_acc (ct,tau,c,m,nc,cs,nid) ! ct(t) = sum w_k c(.,k)                         
...
c 1      format(i5,i4,1p3e14.5,' bdf')                                                         
         ...
         do itau = 1,ntaubd ! ntaubd=number of RK4 substeps (typ. 1 or 2)                      
            ...
            call conv_rhs_acc(r1,p0,ct,bmsk,gsl)         !  STAGE 1                            
...
c-----------------------------------------------------------------------                       
      subroutine int_vel_acc(c_t,t0,c,n,nc,ct,nid)
...
!!$ACC  DATA COPYIN(ct(0:nc))                                                                  
!!$ACC& COPYOUT(wt(0:lwtmax))                                                                  
      call fd_weights_full(t0,ct(0),no,0,wt)  ! interpolation weights                          
!!$ACC END DATA                                                                                

c      call rzero(c_t,n)                                                                       

!!$ACC  DATA COPYIN(c(1:n,0:nc),wt(0:no))                                                      
!!$ACC& COPYOUT(c_t(1:n))                                                                      
...
c-----------------------------------------------------------------------                       
      subroutine conv_rhs_acc (du,u,c,bmsk,gsl)
c                                                                                              
      include 'SIZE'
      include 'TOTAL'
c                                                                                              
...
c                                                                                              
      if (ifconv) then
         ...
         if (ifcons) then
           if (if3d     ) call convop_cons_3d_acc (du,u,c,nx1,nxd,nelv)
           if (.not.if3d) call convop_cons_2d (du,u,c,nx1,nxd,nelv)
         else
           if (if3d     ) call convop_fst_3d_acc  (du,u,c,nx1,nxd,nelv)
           if (.not.if3d) call convop_fst_2d  (du,u,c,nx1,nxd,nelv)
         endif
...
c-----------------------------------------------------------------------                       
      subroutine convop_cons_3d_acc(du,u,c,mx,md,nel) ! Conservation form                      
...
c     Assumes that current convecting field is on dealias mesh, in c()                         
      ...
      write(*,*) "convop_cons_3d is implemented yet"
      stop
...
c-----------------------------------------------------------------------                       
      subroutine convop_fst_3d_acc(du,u,c,mx,md,nel)
c                                                                                              
      include 'SIZE'
c                                                                                              
c     apply convecting field c to scalar field u                                               
...
c                                                                                              
      ...
      ltest = md*mx
      if (if3d) ltest = md*md*mx + mx*md*md
      ...
      if ( i .ne. 1 .or. ip .ne. 1 ) then
         write(*,*) "i and ip should be 1"
         stop
      endif
...
!$ACC DATA                                                                                     
      do e=1,nel
c         call grad_rstd(ur,us,ut,u(1,e),mx,md,if3d,ud)                                        
c      subroutine grad_rstd(ur,us,ut,u,mx,md,if3d,ju) ! GLL->GL grad                           
c         call intp_rstd(ud,u,mx,md,if3d,0) ! 0 = forward                                      


c                                                                                              
c         call specmpn       (ud,md,u,mx,jgl(i),jgt(i),if3d,w,ldw)                             
c      subroutine specmpn_acc( b,nb,a,na,    ba,    ab,if3d,w,ldw)                             
         mxb = mx*md
         mdb = md*md
         call mxm(jgl(i),md,u(1,e),mx,w,mx*mx)
         k=1
         l=mx*mx*md + 1
         do iz=1,mx
            call mxm(w(k),md,jgt(i),mx,w(l),md)
            k=k+mxb
            l=l+mdb
         enddo
         l=mx*mx*md + 1
         call mxm(w(l),mdb,jgt(i),mx,ud(1),md)

         m0 = md-1
         call mxm(dg(ip),md,ud,md,ur,md*md)
         k=1
         l=1
         do iz=1,md
            call mxm(ud(k),md,dgt(ip),md,us(l),md)
            k=k+md*md
            l=l+md*md
         enddo
         call mxm(ud,md*md,dgt(ip),md,ut,md)
c                                                                                              
c      enddo                                                                                   
...
c              C has the mass matrix factored in per (4.8.5), p. 227, DFM.                     
               ud(i) = c(i,e,1)*ur(i)+c(i,e,2)*us(i)+c(i,e,3)*ut(i)
            enddo
c            call intp_rstd(du(1,e),ud,mx,md,if3d,1) ! 1 --> transpose                         
c      subroutine intp_rstd_acc(ju,u,mx,md,if3d,idir) ! GLL->GL interpolation                  

c               call specmpn(du(1,e),mx,ud,md,jgt(i),jgl(i),if3d,w,ldw)                        
c      subroutine specmpn   (      b,nb, a,na,    ba,    ab,if3d,w,ldw)                        

c                                                                                              
            mdb = md*mx
            mxb = mx*mx
            call mxm(jgt(i),mx,ud,md,w,md*md)
            k=1
            l=md*md*mx + 1
            do iz=1,md
               call mxm(w(k),mx,jgl(i),md,w(l),mx)
               k=k+mdb
               l=l+mxb
            enddo
            l=md*md*mx + 1
            call mxm(w(l),mxb,jgl(i),md,du(1,e),mx)
c         enddo                                                                                
c                                                       
      else !! ifd = .false.                                                                    
c         do e = 1,nel                                                                         
...
c-----------------------------------------------------------------------                       
      subroutine grad_rstd_acc(ur,us,ut,u,mx,md,if3d,ju) ! GLL->GL grad                        
...
c-----------------------------------------------------------------------                       
      subroutine local_grad3_acc(ur,us,ut,u,N,e,D,Dt)
...
c-----------------------------------------------------------------------                       
      subroutine intp_rstd_acc(ju,u,mx,md,if3d,idir) ! GLL->GL interpolation                   
...
c                                                                                              
      if (idir.eq.0) then
         call specmpn_acc(ju,md,u,mx,jgl(i),jgt(i),if3d,w,ldw)
      else
         call specmpn_acc(ju,mx,u,md,jgt(i),jgl(i),if3d,w,ldw)
      endif
c                                                                                              
      return
      end
...
c-----------------------------------------------------------------------                       
      subroutine intp_rstd1_acc(i,mx,md) ! GLL->GL interpolation                               
...
c-----------------------------------------------------------------------                       
      subroutine intp_rstd2_acc(i,ju,u,mx,md,if3d,idir) ! GLL->GL interpolation                
...
c      call lim_chk(md,ld,'md   ','ld   ','grad_rstd ')                                        
c      call lim_chk(mx,ld,'mx   ','ld   ','grad_rstd ')                                        

      ldw = 2*(ld**ldim)

c      call get_int_ptr (i,mx,md)                                                              
c                                                                                              
      if (idir.eq.0) then
         call specmpn_acc(ju,md,u,mx,jgl(i),jgt(i),if3d,w,ldw)
      else
...
c-----------------------------------------------------------------------                       
      subroutine set_convect_new_acc(cr,cs,ct,ux,uy,uz)
C                                                                                              
C     Put vxd,vyd,vzd into rst form on fine mesh                                               
C                                                                                              
C     For rst form, see eq. (4.8.5) in Deville, Fischer, Mund (2002).                          
C                                                                                              
      ...
      common /scrcv/ fx(ltd,lelv),fy(ltd,lelv),fz(ltd,lelv)
     $             , ur(ltd),us(ltd),ut(ltd)
     $             , tr(ltd,3),uf(ltd)

      integer i, e

      call set_dealias_rx_acc
      ...
      if (if3d) then
         ...
         do e=1,nelv
c        Map coarse velocity to fine mesh (C-->F)                                              
            call intp_rstd1_acc(i,nx1,nxd) ! GLL->GL interpolation                             
            call intp_rstd2_acc(i,fx(1,e),ux(1,e),nx1,nxd,if3d,0) ! 0 --> forward              
            call intp_rstd2_acc(i,fy(1,e),uy(1,e),nx1,nxd,if3d,0) ! 0 --> forward              
            call intp_rstd2_acc(i,fz(1,e),uz(1,e),nx1,nxd,if3d,0) ! 0 --> forward              

c            call intp_rstd_acc(fx,ux(1,e),nx1,nxd,if3d,0) ! 0 --> forward                     
c            call intp_rstd_acc(fy,uy(1,e),nx1,nxd,if3d,0) ! 0 --> forward                     
c            call intp_rstd_acc(fz,uz(1,e),nx1,nxd,if3d,0) ! 0 --> forward                     

c        Convert convector F to r-s-t coordinates                                              
            do i=1,nxyzd
               cr(i,e)=rx(i,1,e)*fx(i,e)
     $              +  rx(i,2,e)*fy(i,e)+rx(i,3,e)*fz(i,e)
               cs(i,e)=rx(i,4,e)*fx(i,e)
     $              +  rx(i,5,e)*fy(i,e)+rx(i,6,e)*fz(i,e)
               ct(i,e)=rx(i,7,e)*fx(i,e)
     $              +  rx(i,8,e)*fy(i,e)+rx(i,9,e)*fz(i,e)
            enddo
         enddo
...
c        Map coarse velocity to fine mesh (C-->F)                                              
            ...
            call intp_rstd_acc(fx,ux(1,e),nx1,nxd,if3d,0) ! 0 --> forward                      
            call intp_rstd_acc(fy,uy(1,e),nx1,nxd,if3d,0) ! 0 --> forward                      
...
c-----------------------------------------------------------------------                       
      subroutine set_ct_cvx_acc(ct,c,m,u,v,w,tau,nc,mc,nelc,ifnew)
      include 'SIZE'
      include 'INPUT'  ! ifcons                                                                
...
c     Save time and map the current velocity to rst coordinates.                               
      ...
      if (ifcons) then
         ...
         call set_convect_new_acc (c(ix,1),c(iy,1),c(iz,1),u,v,w)
      endif
...
c-----------------------------------------------------------------------                       
      subroutine set_conv_char_acc(ct,c,ux,uy,uz,nelc,tau,ifnew)
      include 'SIZE'
      include 'TSTEP'
...
c-----------------------------------------------------------------------                       
      subroutine setup_convect_acc(igeom)
      include 'SIZE'
      include 'TOTAL'
      logical ifnew
      ...
      if (ifchar) then
         ...
         call set_conv_char_acc(ct_vx,c_vx,vx,vy,vz,nelc,time,ifnew)
      else
