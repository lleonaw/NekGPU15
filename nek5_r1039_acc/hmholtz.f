...
#ifdef _OPENACC
!$ACC UPDATE DEVICE(KTYPE) ASYNC(1)
!$ACC WAIT  
#endif 
...
C
      parameter        (lg=lx1*ly1*lz1*lelt)
c      real x(1),f(1),h1(1),h2(1),mask(1),mult(1),binv(1)
...
c----------------------------------------------------------------------- 
      ...
      real d (nx1,ny1,nz1,nel)
      real h1(nx1,ny1,nz1,nel)
      real h2(nx1,ny1,nz1,nel)

      real h1b,h2b,vol,vl1,vl2,vl3,den

      integer ie,i1,i2,i3,k1,k2,k3,nel,nxyz
      nxyz = nx1*ny1*nz1

!$ACC PARALLEL LOOP PRIVATE(h1b,h2b,k1,k2,k3,vol,vl1,vl2,vl3)
         do ie=1,nel
            h1b = 0
            h2b = 0
            
!$ACC LOOP REDUCTION(+:h1b)
            do i3=1,nz1
               do i2=1,ny1
                  do i1=1,nx1
                     h1b = h1b+h1(i1,i2,i3,ie)
                  enddo
               enddo
            enddo
            h1b = h1b / nxyz

!$ACC LOOP REDUCTION(+:h2b)
            do i3=1,nz1
               do i2=1,ny1
!$ACC LOOP SEQ
                  do i1=1,nx1
                     h2b = h1b+h2(i1,i2,i3,ie)
                  enddo
               enddo
            enddo
            h2b = h2b / nxyz
...
C
c=======================================================================
      subroutine axhelm2_acc (au,u,helm1,helm2,imesh,isd)
C------------------------------------------------------------------
C
C     Compute the (Helmholtz) matrix-vector product,
C     AU = helm1*[A]u + helm2*[B]u, for NEL elements.
