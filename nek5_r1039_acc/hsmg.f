...
#ifdef _OPENACC
      call h1mg_setup_dssum_acc  ! set direct stiffness summation handles
#endif
...
c----------------------------------------------------------------------                        
      ...
      do l=1,mg_lmax  ! set up direct stiffness summation for each level                       
         ...
         ny=mg_nh(l)
         nz=mg_nhz(l)
         call setupds_hsmg_acc(l,mg_gsh_handle_acc(l,mg_fld),nx,ny,nz
     $                ,        nelv,nelgv,vertex,glo_num)
         ...
         if(.not.if3d) nz=1
         l2 = l+mg_lmax
         call setupds_hsmg_acc(l2,mg_gsh_schwarz_handle_acc(l,mg_fld)
...
c----------------------------------------------------------------------                        
      subroutine hsmg_tnsr1_3d_acc(v,nv,nu,A,B,C) ! v = [C (x) B (x) A] u                      
      include 'SIZE'
...
c------------------------------------------   T  -----------------------                       
      subroutine h1mg_rstr_acc(r,l,ifdssum) ! r =J r,   l is coarse level                      
      include 'SIZE'
      include 'HSMG'
      include 'INPUT'
...
c----------------------------------------------------------------------                        
      ...
      call h1mg_mask_acc  (r,mg_imask(pm),nelfld(ifield))  ! Zero Dirichlet nodes              
...
c-----------------------------------------------------------------------                       
      subroutine h1mg_solve_acc(z,rhs,if_hybrid)  !  Solve preconditioner: Mz=rhs              
...
!        OVERLAPPING Schwarz exchange and solve:                                               
         ...
         call h1mg_schwarz_acc(e(is),r,sigma,l)       ! e := sigma W M       r                 
                                                      !  l            Schwarz l                
         if(if_hybrid)call h1mg_axm(r,e(is),op,om,l,w)! r  := r - A e                          
                                                      !  l           l                         
      enddo
      ...
      n1 = mg_h1_n(1,mg_fld)
!$ACC UPDATE HOST(r(1:n1)) ASYNC(1)
!$ACC WAIT
      call hsmg_coarse_solve ( e(is) , r )        ! e  := A   r                                
!$ACC UPDATE DEVICE(e(is:is+n1)) ASYNC(2)
!$ACC WAIT
...
c----------------------------------------------------------------------                        
      subroutine hsmg_coarse_solve_acc2(e,r)
      include 'SIZE'
      include 'DOMAIN'
      include 'ESOLV'
      include 'GEOM'
