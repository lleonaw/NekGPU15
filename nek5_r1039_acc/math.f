...
!$ACC PARALLEL LOOP REDUCTION(+:TMP) VECTOR_LENGTH(128)
      DO 10 I=1,N
         TMP = TMP + A(I)*B(I)*MULT(I)
 10   CONTINUE
!$ACC END PARALLEL LOOP
...
#endif
C
!$ACC DATA PRESENT(A(1:n),B(1:n),C(1:n),D(1:n))
!$ACC PARALLEL LOOP
      DO 100 I=1,N
         A(I)=A(I)+B(I)*C(I)*D(I)
  100 CONTINUE
...
c
      ...
      is_present= ( acc_is_present(x,1).and.
     $              acc_is_present(y,1).and.
     $              acc_is_present(z,1) )
 
      ds = 0.0

!$ACC DATA PRESENT(x(1:n),y(1:n),z(1:n)) if (is_present)
