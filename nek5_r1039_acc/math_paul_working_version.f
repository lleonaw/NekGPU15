...
#endif
C
!$ACC DATA PRESENT(a(1:n),b(1:n),c(1:n),d(1:n))  if (is_present)
!$ACC PARALLEL LOOP  if (is_present)
      DO 100 I=1,N
         a(i)=a(i)+b(i)*c(i)*d(i)
  100 CONTINUE
...
c-----------------------------------------------------------------------
      ...
      include 'OPCTR'
      logical is_present
      is_present = acc_is_present(vec,1)
...
C
      SUM = 0.
C
!$ACC DATA PRESENT(vec(1:n))  if (is_present)
!$ACC PARALLEL LOOP REDUCTION(+:SUM)  if (is_present)
      DO 100 I=1,N
         sum=sum+vec(i)
 100  CONTINUE
!$ACC END PARALLEL LOOP
...
c     Perform inner-product  a*a*b*c
...
!$ACC DATA PRESENT(a(1:n),b(1:n),c(1:n))  if (is_present)
!$ACC PARALLEL LOOP REDUCTION(+:ds)       if (is_present)
      do i=1,n
         ds=ds+a(i)*a(i)*b(i)*c(i)
      enddo
!$ACC END PARALLEL LOOP
!$ACC END DATA
