...
c-----------------------------------------------------------------------
      subroutine opdssum_acc (a,b,c)! NOTE: opdssum works on FLUID/MHD arrays only!
...
C   ifsplit .eq. T and .not. ifaxis  
      if (ifsplit .and. .not.ifaxis) then
         call wgradm1_acc(out1,out2,out3,inp,nelv) ! weak grad on FLUID mesh
         return
      endif
...
!$ACC PARALLEL LOOP COLLAPSE(4) VECTOR_LENGTH(256) WORKER GANG VECTOR 
         do e=1,nelv
            do k=1,nz2
            do j=1,ny2
            do i=1,nx2
