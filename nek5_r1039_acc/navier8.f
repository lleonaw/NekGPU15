...
!$ACC DATA PRESENT(h1_basis_acc,h1_basist_acc)
      call transpose(h1_basist_acc,2,h1_basis_acc,lx1)
!$ACC UPDATE DEVICE(h1_basis_acc, h1_basist_acc) ASYNC(1)
!$ACC WAIT
!$ACC END DATA
#endif
...
c-----------------------------------------------------------------------
c
      subroutine specmpn_acc(b,nb,a,na,ba,ab,if3d,w,ldw)
C
C     -  Spectral interpolation from A to B via tensor products
C     -  scratch arrays: w(na*na*nb + nb*nb*na)
C
