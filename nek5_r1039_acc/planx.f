...
#ifdef _OPENACC
c-----------------------------------------------------------------------
      subroutine ophinvpr_acc(ot1,ot2,ot3,in1,in2,in3,h1,h2,tolh,nmxi)
C
C     OT = (H1*A+H2*B)-1 * IN  (implicit)
c     VELOCITY 
C
      ...
      mprev=min(mprev,mxprev)
      if (mprev.le.0 .or. istep.le.istart.or.istart.le.0) then
         call ophinv_acc  (ot1,ot2,ot3,in1,in2,in3,h1,h2,tolh,nmxi)
         return
      endif
     ...
     $                  TOLH,NMXi,MATMOD)
      ELSEIF(ifield.eq.1) then
         CALL hmzpf2_acc ('VELX',OT1,IN1,H1,H2,V1MASK,VMULT,
     $                                   IMESH,TOLH,NMXi,1)
         CALL hmzpf2_acc ('VELY',OT2,IN2,H1,H2,V2MASK,VMULT,
     $                                   IMESH,TOLH,NMXi,2)
         IF (NDIM.EQ.3) 
     $   CALL hmzpf2_acc ('VELZ',OT3,IN3,H1,H2,V3MASK,VMULT,
     $                                   IMESH,TOLH,NMXi,3)
      ELSE 
         write(*,*) "OPHINV_ACC is not implemented"
         stop
         CALL hmzpf2 (' BX ',OT1,IN1,H1,H2,B1MASK,VMULT,
     $                                   IMESH,TOLH,NMXi,1)
         CALL hmzpf2 (' BY ',OT2,IN2,H1,H2,B2MASK,VMULT,
...
c-----------------------------------------------------------------------
      subroutine hmzpf2_acc(nm,u,rhs,h1,h2,mask,mult,imsh,tol,mxit,isd)
      include 'SIZE'
      include 'INPUT'
      include 'MASS'
      character*4 nm
c
      ...
      call projh2 (rhs,h1,h2,mult,mask,isd,imsh)
      if (imsh.eq.1) then
        call hmhzpf_acc (nm,u,rhs,h1,h2,mask,mult,imsh,tol
     $        ,          mxit,isd,binvm1)
      else
        call hmhzpf_acc (nm,u,rhs,h1,h2,mask,mult,imsh,tol
     $        ,          mxit,isd,bintm1)
